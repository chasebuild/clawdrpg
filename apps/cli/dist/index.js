...
program
    ...
    if (!supabaseUrl || !supabaseAnonKey) {
        console.error(picocolors_1.default.red('Error: Supabase credentials missing. Set SUPABASE_URL and SUPABASE_ANON_KEY env vars.'));
        process.exit(1);
    }
    const supabase = (0, supabase_js_1.createClient)(supabaseUrl, supabaseAnonKey);
    const workdir = path_1.default.resolve(options.workdir);
...
Looking for character: ${slug}...`));
    try {
        const { data: character, error } = await supabase
            ...
            .single();
        if (error || !character) {
            console.error(picocolors_1.default.red(`Error: Character with slug "${slug}" not found.`));
            process.exit(1);
        }
        console.log(picocolors_1.default.green(`Found ${picocolors_1.default.bold(character.name)}! Installing to ${workdir}...`));
...
`;
        await fs_extra_1.default.writeFile(path_1.default.join(workdir, 'IDENTITY.md'), identityContent);
        console.log(picocolors_1.default.blue('✓ Created IDENTITY.md'));
        // 2. Write SOUL.md
        await fs_extra_1.default.writeFile(path_1.default.join(workdir, 'SOUL.md'), character.soul_content);
        console.log(picocolors_1.default.blue('✓ Created SOUL.md'));
...
- When in doubt, ask.
`;
        await fs_extra_1.default.writeFile(path_1.default.join(workdir, 'AGENTS.md'), agentsContent);
        console.log(picocolors_1.default.blue('✓ Created AGENTS.md'));
        // 4. Write TOOLS.md
...
`;
        await fs_extra_1.default.writeFile(path_1.default.join(workdir, 'TOOLS.md'), toolsContent);
        console.log(picocolors_1.default.blue('✓ Created TOOLS.md'));
...
`;
        await fs_extra_1.default.writeFile(path_1.default.join(workdir, 'USER.md'), userContent);
        console.log(picocolors_1.default.blue('✓ Created USER.md'));
        // 6. Write HEARTBEAT.md
        await fs_extra_1.default.writeFile(path_1.default.join(workdir, 'HEARTBEAT.md'), '# HEARTBEAT.md\n\n# Add periodic tasks here.\n');
        console.log(picocolors_1.default.blue('✓ Created HEARTBEAT.md'));
...
`;
        await fs_extra_1.default.writeFile(path_1.default.join(workdir, 'BOOTSTRAP.md'), bootstrapContent);
        console.log(picocolors_1.default.blue('✓ Created BOOTSTRAP.md'));
        // 8. Handle Skills (Placeholder/Info)
        if (character.skills && character.skills.length > 0) {
            console.log(picocolors_1.default.yellow(`\nCharacter requires the following skills:`));
            character.skills.forEach((s) => console.log(` - ${s}`));
            console.log(picocolors_1.default.dim(`\nNote: Skill installation via clawhub coming soon!`));
        }
        console.log(picocolors_1.default.green(`
🚀 ${picocolors_1.default.bold(character.name)} has been installed successfully!
`));
    }
    catch (err) {
        console.error(picocolors_1.default.red('An unexpected error occurred:'), err);
        process.exit(1);
    }
});
program
    ...
    try {
        ...
        console.error(picocolors_1.default.red('Error fetching skills:'), err);
    }
});
program.parse();
