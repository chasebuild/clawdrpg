...
import { Sparkles, User, FileText, Package, Save, Plus, X, Wand2, RefreshCw, Info, Search, Cpu, Terminal, Zap, ShieldAlert, History } from 'lucide-react';
...
export default function BuilderPage() {
  ...
  const [selectedSkillForInfo, setSelectedSkillForInfo] = useState<typeof ALL_SKILLS_DATA[0] | null>(null);
    ...
    <div className="flex min-h-screen flex-col bg-black text-foreground font-sans cyber-grid selection:bg-accent selection:text-black">
      <header className="border-b border-white/10 bg-black/60 backdrop-blur-md px-10 py-6 flex justify-between items-center sticky top-0 z-40">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black tracking-tighter uppercase italic glow-text">ClawdRPG_Builder</h1>
            <div className="text-[9px] font-mono text-white/30 uppercase tracking-[0.3em]">System_Status: Operational // Session_Active</div>
          </div>
        </div>
        ...
        <aside className="w-80 border-r border-white/10 bg-black/40 p-8 flex flex-col gap-10 overflow-y-auto">
          <div>
            <div className="flex items-center gap-2 mb-6">
              ...
              <h3 className="text-[10px] font-black text-white/40 uppercase tracking-[0.2em]">Neural_Generator</h3>
            </div>
          ...
          <nav className="space-y-2">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h3 className="text-[10px] font-black text-white/40 uppercase tracking-[0.2em]">Manual_Override</h3>
            </div>
            {[
              ...
              >
                ...
                {activeTab === tab.id && <div className="w-1.5 h-1.5 bg-accent rounded-full animate-ping" />}
              </button>
            ))}
          </nav>
          ...
          <div>
            <div className="flex items-center gap-2 mb-6">
              ...
              <h3 className="text-[10px] font-black text-white/40 uppercase tracking-[0.2em]">Archetypes</h3>
            </div>
              ...
              {Object.entries(ARCHETYPES).map(([key, arc]) => (
                ...
                >
                  <span className="flex items-center gap-2">
                    <span className="text-lg grayscale group-hover:grayscale-0 transition-all">{arc.emoji}</span>
          ...
          <div className="max-w-4xl mx-auto space-y-12 pb-24">
            ...
            {activeTab === 'identity' && (
              <section className="cyber-panel rounded-sm p-10 space-y-10">
                <div>
                  <h2 className="text-3xl font-black italic uppercase tracking-tighter mb-2 glow-text">Character_Identity</h2>
                  <p className="text-white/40 text-xs font-mono tracking-widest uppercase">System Initialization: visual_markers and naming_conventions.</p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                  <div className="space-y-6">
                    <div>
                      <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-accent/60 mb-3 italic">Display_Name</label>
                      <input 
                        type="text" 
                        value={formData.name}
                    ...
                    <div>
                      <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-accent/60 mb-3 italic">Identifier_Slug</label>
                  ...
                  <div className="flex flex-col items-center justify-center p-8 border-2 border-dashed border-white/10 rounded-sm bg-white/5">
                    <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-white/40 mb-6 italic">Avatar_Index</label>
                    ...
                    <p className="mt-6 text-[9px] uppercase font-black text-white/20 tracking-[0.3em] italic">Click to cycle marker</p>
                  </div>
                </div>

                <div>
                  <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-accent/60 mb-3 italic">External_Asset_URI (Optional)</label>
                  <input 
                    type="text" 
            ...
            {activeTab === 'soul' && (
              <section className="space-y-8">
                <div className="cyber-panel rounded-sm p-10">
                  <h2 className="text-3xl font-black italic uppercase tracking-tighter mb-2 glow-text">Soul_Core</h2>
                  <p className="text-white/40 text-xs font-mono tracking-widest uppercase mb-10">Neural Mapping: heuristics, motivations, and operational_vibe.</p>
                  
                  <div className="space-y-8">
                    <div>
                      <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-accent/60 mb-3 italic">Persona_Matrix</label>
                      <textarea 
                    ...
                    <div>
                      <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-accent/60 mb-3 italic">Intrinsic_Motivation</label>
                      <textarea 
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-accent/60 mb-3 italic">Acoustic_Tone</label>
                        <input 
                          type="text" 
                      ...
                      <div>
                        <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-accent/60 mb-3 italic">Diction_Style</label>
                        <input 
                          type="text" 
                    ...
                    <div>
                      <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-accent/60 mb-3 italic">Persistence_Logic</label>
                      <textarea 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="cyber-panel rounded-sm p-8">
                    <div className="flex justify-between items-center mb-8">
                      <h3 className="text-xs font-black uppercase italic tracking-widest glow-text">Core_Truths</h3>
                      <button 
                  ...
                  <div className="cyber-panel rounded-sm p-8">
                    <div className="flex justify-between items-center mb-8">
                      <h3 className="text-xs font-black uppercase italic tracking-widest glow-text">Heuristic_Boundaries</h3>
                      <button 
            ...
            {activeTab === 'skills' && (
              <section className="space-y-8">
                <div className="cyber-panel rounded-sm p-10">
                  <div className="mb-10">
                    <h2 className="text-3xl font-black italic uppercase tracking-tighter mb-2 glow-text">Abilities_Registry</h2>
                    <p className="text-white/40 text-xs font-mono tracking-widest uppercase">Injecting external functional modules into Character_Core.</p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[600px] overflow-y-auto pr-4 custom-scrollbar">
                        ...
                        >
                          ...
                          >
                            <div className="flex items-center gap-2 mb-1">
                              <span className="text-[8px] font-black text-white/30 uppercase italic tracking-widest">Name:</span>
                              <span className={`font-black text-xs uppercase italic truncate tracking-wider ${isSelected ? 'text-accent glow-text' : 'text-white'}`}>
                                {skill.name}
                              </span>
                            </div>
                            <div className="flex items-start gap-2">
                              <span className="text-[8px] font-black text-white/20 uppercase italic tracking-widest mt-0.5">Desc:</span>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            >
                              {isSelected ? <Zap className="w-4 h-4 animate-pulse" /> : <Plus className="w-4 h-4" />}
                            </button>
                          </div>
                        </div>
                      );
                    ...
                    {filteredSkills.length === 0 && (
                      <div className="col-span-full py-24 text-center text-white/20 font-mono italic">
                        ...
                        <p className="uppercase tracking-[0.2em] text-xs">No matching abilities detected in registry.</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="cyber-panel rounded-sm p-8 border-accent/20">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {formData.selected_skills.map(slug => (
                      <div key={slug} className="px-4 py-2 bg-accent/10 border border-accent/30 rounded-sm text-[10px] font-black uppercase italic text-accent flex items-center gap-3">
                        {slug}
                        <button onClick={() => toggleSkill(slug)} className="hover:text-red-500 transition-colors">
                          <X className="w-4 h-4" />
                        </button>
                      </div>
                    ))}
                    {formData.selected_skills.length === 0 && (
                      <p className="text-[10px] font-mono text-white/20 uppercase tracking-widest italic">Character currently has no special abilities.</p>
                    )}
                  </div>
                </div>
              </section>
